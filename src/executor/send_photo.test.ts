...
import { sendPhoto } from './send_photo.ts';

describe('sendPhoto', () => {
	it('calls sendPhoto API with correct chat_id and photo as string file_id', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 1 });
		await sendPhoto(mock.bot, 100, 'file_id_abc');
		...
		assert.equal(params.photo, 'file_id_abc');
	});
	...
	it('wraps Buffer in InputFile', async () => {
		...
		await sendPhoto(mock.bot, 1, buf);
		const call = mock.calls[0];
		assert.ok(call !== undefined);
		const params = call.args[0] as Record<string, unknown>;
	...
	it('wraps { url } in InputFile', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 1 });
		await sendPhoto(mock.bot, 1, { url: 'https://example.com/img.jpg' });
		const call = mock.calls[0];
		assert.ok(call !== undefined);
		const params = call.args[0] as Record<string, unknown>;
		assert.ok(params.photo instanceof InputFile);
	...
	it('forwards caption', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 1 });
		await sendPhoto(mock.bot, 1, 'fid', { caption: 'hello' });
		const call = mock.calls[0];
		assert.ok(call !== undefined);
		const params = call.args[0] as Record<string, unknown>;
	...
	it('forwards parse_mode', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 1 });
		await sendPhoto(mock.bot, 1, 'fid', { parseMode: 'HTML' });
		const call = mock.calls[0];
		assert.ok(call !== undefined);
		const params = call.args[0] as Record<string, unknown>;
	...
	it('forwards reply_to_message_id', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 1 });
		await sendPhoto(mock.bot, 1, 'fid', { replyToMessageId: 42 });
		const call = mock.calls[0];
		assert.ok(call !== undefined);
		const params = call.args[0] as Record<string, unknown>;
	...
	it('returns { messageId } from API response', async () => {
		const mock = createMockGrammy();
		mock.setResponse('sendPhoto', { message_id: 77 });
		const result = await sendPhoto(mock.bot, 1, 'fid');
